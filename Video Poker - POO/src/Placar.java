...
public class Placar {
	...
	//pra teste
	public static int mao[] = new int[5];
	public static int naipe[]= new int[5];
	...
	public void Pontuacao(){
		...
		for(int i = 2 ; i< 15 ; ++i){
			for(int j = 0; j<5 ; ++j){
				if(i == mao[j]/*mao[j].GetNumero*/)			//compara se a posicao do contador for igual ao numero da carta
					++vet_cont[i];
			}
		}
		...
		for(int i = 0 ; i<5  ; ++i){
			for(int j = 0; j<5 ; ++j){
				if(i == naipe[j]/*mao[j].getNaipe*/)		//compara se o naipe do contador for igual ao naipe da carta
					++cont_naipe[i];
			}
		}
		...
		//verifica se h� Royal Straight Flush
		...
		for (int i =2; i<15; ++i){			
			...
			if(aux == 4){
				...
				//verifica se ha um par
					...
					if(flagpar == true )		//verifica se h� dois pares
						flagpar2 = true;
					else 
